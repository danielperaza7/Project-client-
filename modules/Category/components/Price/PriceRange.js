...
export const renderPriceMap = (tiers, map) => {
  ...
  if (!map || isEmpty(map) || keys.lenth < 1) {
    // no map
    return null;
  }
...
export const renderPrice = (tier_prices, groupId, origin_prices, isKit = false) => {
  // console.log('renderPrice', tier_prices, groupId, origin_prices);
  ...
  if (match && (origin_prices.min !== min && origin_prices.max !== max)) {
    // show original price and current price
    if (isKit) {
      return (
        <span>
...
export const groupByGroupId = (productList) => {
  ...
  for (const price of uniformPrice) {
    if (Object.keys(groupMap).length > 0) {
      ...
      // check previous version 69a1243520752ffdf217d7c45a3c34d2be505e97, here was key var which was always undefined?
      const obj = { customer_group_id: undefined, value: price, qty: 1 };
