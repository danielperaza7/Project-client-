...
import {
  ...
  StorePage_ALBUM_2,
  TURN_MODEL_OVER
} from "../../../../config/Customer/StorePageConfig";
...
class StorePage extends React.PureComponent {
  ...
  componentDidMount() {
    axios({
      method: "get",
      url: `${HIDDEN_FIGURE_URL}/v1/category?customer_group_id=0&psize=40&store=et&category=category/store-new-arrival&country=us`,
      headers: {}
    })
      .then((response) => {
        if (response.status !== 200) {
          ...
          this.setState({ products: response.data.data.products.slice(0, 16) });
        } else {
    ...
    axios({
      method: "get",
      url: `${HIDDEN_FIGURE_URL}/v1/category?customer_group_id=0&psize=40&store=et&category=category/store-best-sellers&country=us`,
      headers: {}
    })
      .then((response) => {
        if (response.status !== 200) {
          ...
          this.setState({
            products: [
              ...this.state.products,
              ...response.data.data.products.slice(0, 16)
            ]
          });
        } else {
          console.log("fetch NBCAM products error");
          ...
          <div className={styles.linkBlock}>
            <MediaQuery minWidth={992} values={{ width: clientMD && clientMD.fakeDeviceWidth }}>
              <div className={styles.singleLink}>
                <a href="#">Our curated list for you</a>
                <i className="ion-play" style={{ fontSize: 16, marginLeft: 10 }} />
              </div>
              <div className={styles.singleLink}>
                <a href="#s2">Earn extra reward points</a>
                <i className="ion-play" style={{ fontSize: 16, marginLeft: 10 }} />
              </div>
            </MediaQuery>
            <MediaQuery maxWidth={991} values={{ width: clientMD && clientMD.fakeDeviceWidth }}>
              ...
              <div className={styles.singleLink}>
                <a href="#">Earn extra reward points</a>
              </div>
            </MediaQuery>
          </div>
        </div>
  ...
  renderBCAMBackground() {
    return (
      <div style={{ lineHeight: "32px", width: "100%" }}>
        <strong>Eve’s Temptation</strong>
        {" "}
opened its first North America store in November
        2017. Dedicated to the Eve’s Temptation Pink Label, it offers a playful mix of
        styles ranging from everyday bras and panties, sleep and loungewear, using a
        variety of whimsical trims, colorful prints, and cheeky, flirtier silhouettes. The
        Pink Label is youthful, energetic, and exudes a feminine sensuality. Never one to
        take herself seriously, the Pink Label woman embraces life and walks with
        confidence.
      </div>
    );
  }
  ...
  //       To further our efforts and raise awareness on breast health, we will be donating<strong> $1 of every bra purchase</strong> during the month of October to a breast cancer related charity to support early breast cancer detection and prevention.
  //     </div>
  //   );
  // }
        ...
        <div className={styles.productListTitle}>
          <MediaQuery minWidth={992} values={{ width: clientMD && clientMD.fakeDeviceWidth }}>
            <img
              ...
              alt="pink heart"
            />
            <span style={{ margin: "0px 30px" }}>Styles in Trend</span>
            <img
      ...
      <div className={styles.rewardPointsBox}>
        <div className={styles.socialMediaContainer}>
          <div className={styles.rewardPointsTitle}>
            ...
            <MediaQuery minWidth={992} values={{ width: clientMD && clientMD.fakeDeviceWidth }}>
              <div>Explore more in our store!</div>
              <img
                src="https://storage.googleapis.com/evesetus/email/NBCAM/pinkheart.svg"
                className={styles.pinkHeart}
                alt="pink heart"
              ...
              <span style={{ margin: "0px 30px" }}>
                Schedule an appointment with our consultant in store
              </span>
              <img
                src="https://storage.googleapis.com/evesetus/email/NBCAM/pinkheart_reverse.svg"
                className={styles.pinkHeart}
            ...
            <MediaQuery maxWidth={991} values={{ width: clientMD && clientMD.fakeDeviceWidth }}>
              <div>Explore more in our store!</div>
              <span>Schedule an appointment with our consultant in store</span>
            </MediaQuery>
          </div>
          <div className={styles.rewardPointsBar}>
            <div className={styles.socialMediaBlock}>
              ...
              <p>Sunday: 11AM-8PM</p>
              <p>Westfield Santa Anita Store,400 S Baldwin Ave J7, Arcadia, CA 91007</p>
            </div>
            <div className={styles.socialMediaBlock}>
              <p style={{ fontSize: 16 }}>
                <i
              ...
              <p>ID: evesclub3</p>
              <p>Tele:6265384546</p>
              <p>Email: arcadiastore@evestemptation.com</p>
            </div>
          </div>
  ...
  renderAlbum(type) {
    const album = type === "wireless" ? StorePage_ALBUM_1 : StorePage_ALBUM_2;
